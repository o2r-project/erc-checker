...
co2=read.csv("CO2incubations_altered.csv") #Data aggregated by treatment
```
...
## !!!! edited here, Temperature from 40 to 39 !!!!
Temp=seq(0,39,1)
W=seq(0,90,0.01)
O=seq(0,20,0.001)
...
plot(subset(xiT,Temperature==27 & Oxygen==20,select=c(Moisture,xi)),col=4,ylim=c(0,3),pch=19,ylab=expression(paste(xi, " (unitless)")),xlab="Water filled pore space (%)",type="b")
lines(subset(xiT,Temperature==35 & Oxygen==20,select=c(Moisture,xi)),col=2,pch=19,type="b")
lines(subset(xiT,Temperature==27 & Oxygen==10,select=c(Moisture,xi)),col=4,pch=4,type="b")
lines(subset(xiT,Temperature==35 & Oxygen==1,select=c(Moisture,xi)),col=2,pch=4,type="b")
abline(h=1,lty=2)
legend("topleft",c("T = 27 C, O = 20 %", "T = 35 C, O = 20%", "T = 27 C, O = 10%", "T = 35 C, O = 1%"), lty=1, col=c(4,2),pch=c(19,19,4,4),bty="n")
