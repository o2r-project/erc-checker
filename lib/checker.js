...
/**
 * Function which gets called by proofBufferEqual. If both buffer are equal, the images are equal and no comparison or
 * resizing is needed.  
 * @param {*} resultingImageBuffers 
 * @param {*} currentBufferOriginal 
 * @param {*} currentBufferReproduced 
 */
function bufferEqual(resultingImageBuffers, currentBufferOriginal, currentBufferReproduced) {
...
/**
 * Function which gets called by prepareImagesForComparison and proofs if the buffer/ images got the same size. 
 * If yes there is no further preparation for comparision needed, if no there is a resizing. 
 * @param {*} currentBufferOriginal 
 * @param {*} currentBufferReproduced 
 * @param {*} index 
 * @param {*} resultingImageBuffers 
 */
function proofBufferSize(currentBufferOriginal, currentBufferReproduced, index, resultingImageBuffers) {
	var dimensionsOriginal;
	var dimensionsReproduced;
	...
	return new Promise(
		function (resolve, reject) {
			...
			if (currentBufferOriginal == currentBufferReproduced) {
				bufferEqual(resultingImageBuffers, currentBufferOriginal, currentBufferReproduced);
			}
			else {
				try {
					dimensionsOriginal = sizeOf(currentBufferOriginal);
					dimensionsReproduced = sizeOf(currentBufferReproduced);
				}
				catch (e) {
				...
				resizeImageIfNecessary(currentBufferOriginal, currentBufferReproduced, dimensionsOriginal, dimensionsReproduced, index)
					.then(function (resolve) {
						...
						console.log(resultingImageBuffers);
					})
...
/**
 * Function which prepares the buffer/ images for comparision. If a resizing is needed the buffer get resized. 
...
function prepareImagesForComparison(twoDimensionalArrayOfBuffers) {
	...
	return new Promise(
		function (resolve, reject) {
			...
			// if original image is empty, resolve 
			...
			originalImageBuffers.forEach(
				...
				function (current, index) {
					...
					proofBufferSize(currentBufferOriginal, currentBufferReproduced, index, resultingImageBuffers)
				}
			);
