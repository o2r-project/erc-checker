		...
		function (resolve, reject) {
			...
			originalImageBuffers.forEach(
				function (current, index) {
					...
					//TODO 
					/*
					if (dimensionsOriginal !== dimensionsReproduced) {
						console.log(continueOnImageSizeDifference); // aus der config holen 
					}
*/
			...
			if (images.length == 0) {
				...
				images.forEach(
					function (current, index) {
						...
						fs.writeFileSync('diff2.png', PNG.sync.write(diff));
