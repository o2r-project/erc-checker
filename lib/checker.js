...
async function resizing(reproducedImageBuffer, dimensionsOriginal) {
	...
	const image = await Jimp.read(reproducedImageBuffer);
	await image.resize(dimensionsOriginal.width, dimensionsOriginal.height);
	await image.writeAsync(`helloooo2`);
	console.log(image);
	return (image);

	/*
	Jimp.read('/home/t_nier01/git/erc-checker/output.png')
		.then(lenna => {
			lenna
				.resize(dimensionsOriginal.width, dimensionsOriginal.height) // resize
	...
	sharp(reproducedImageBuffer)
		...
		.then(reproducedImageBufferResized => {
			...
			resolve(reproducedImageBufferResized);
		})
		.catch(e => {
		...
		*/
	/*
	jimp.read(reproducedImageBuffer)
		.resize(dimensionsOriginal.width, dimensionsOriginal.height)
	...
	return new Promise(
		function (resolve, reject) {
			if (dimensionsOriginal.width != dimensionsReproduced.width || dimensionsOriginal.height != dimensionsReproduced.height) {
				resizing(reproducedImageBuffer, dimensionsOriginal)
					.then(function (reproducedImageBufferResized) {
...
function runBlinkDiff(images) {
	...
	console.log(images[0].reproducedImage.buffer);
