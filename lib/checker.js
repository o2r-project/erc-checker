			...
			try {
				...
				*/
				if (bufferedImagesOriginal.length < bufferedImagesReproduced.length) {
					reject([new Error("There are more reproduced images than original images (" + bufferedImagesOriginal.length + " originalImageBuffers and " + bufferedImagesReproduced.length + " reproducedImageBuffers)."), undefined]);
					metadata.errors.push("There are more reproduced images than original images (" + bufferedImagesOriginal.length + " originalImageBuffers and " + bufferedImagesReproduced.length + " reproducedImageBuffers).");
				}
...
/**
 * Function which gets called if the current buffers are equal and bring them then together in the output structure. 
 * @param currentBufferOriginal 
 * @param currentBufferReproduced 
 */
function equalBuffers(currentBufferOriginal, currentBufferReproduced) {
	return {
		originalImage: {
...
/**
 * Function which proofs for each of the buffer-couples (original and resized) if they got the same size. 
 * If not the reproduced image buffer gets resized. It takes the originalImageBuffers and the reproducedImageBuffers. 
 * It returns one resultingImageBuffers with the originalImageBuffers and the reproducedImageBuffers. The 
 * reproducedImageBuffers are resized if needed. 
 * @param originalImageBuffers 
 * @param reproducedImageBuffers 
 */
function proofResizingNeeded(originalImageBuffers, reproducedImageBuffers) {
		...
		function (resolve, reject) {
			...
			originalImageBuffers.forEach(
				function (current, index) {
					...
					var currentBufferReproduced = reproducedImageBuffers[index];

					if (currentBufferOriginal === currentBufferReproduced) {
						debugCompare("Images with index %s are equal.", index);
						resultingImageBuffers.images[index] = equalBuffers(currentBufferOriginal, currentBufferReproduced);
					}
					else {
						try {
...
function prepareImagesForComparison(twoDimensionalArrayOfBuffers) {
	console.log(twoDimensionalArrayOfBuffers);

	return new Promise(
		function (resolve, reject) {
			var originalImageBuffers = twoDimensionalArrayOfBuffers[0];
			var reproducedImageBuffers = twoDimensionalArrayOfBuffers[1];

			if (originalImageBuffers.length == 0) {
				resolve({ images: [] });
			}
			else {
				proofResizingNeeded(originalImageBuffers, reproducedImageBuffers)
					.then(function (resolveProofResizingNeeded) {
						resolveProofResizingNeeded.images.forEach(
							function (current, index) {
						...
						resolve(resolveProofResizingNeeded.images); 
					}
					);
			}
		}
