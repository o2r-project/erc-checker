...
function resizeImageIfNecessary(originalImageBuffer, reproducedImageBuffer, dimensionsOriginal, dimensionsReproduced, index) {
	...
	console.log(dimensionsOriginal);
	console.log(dimensionsReproduced);
			...
			if (images.length == 0) {
				...
				images.forEach(
					function (current, index) {
						...
						const img1 = current.originalImage.buffer;
						const img2 = current.reproducedImage.buffer;

						// compared to images from the old o2r platformm 
						...
						const img3Data = img3.data;

						// width an height used for the produced comparision (the dimension of the both campared and the result image need to be exactly the same)
						let width = 1344;
						let height = 960;
						...
						// pixelmatch(img1, img2, diff2.data, width, height, {threshold: 0.1});					
